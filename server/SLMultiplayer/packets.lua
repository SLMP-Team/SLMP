function Packet_Connect(bitStream, pAddress, pPort)
  ...
  for i = 1, #SPool.sPickups do
    SLNet.setWritePointerOffset(bs, 2)
    SLNet.writeInt16(bs, SPool.sPickups[i].pickupid)
    SLNet.writeInt16(bs, SPool.sPickups[i].modelid)
    SLNet.writeInt8(bs, SPool.sPickups[i].pickuptype)
    SPool.sendRPC(bs, pAddress, pPort)
  end
  SLNet.deleteBitStream(bs)
...
function Packet_OnFoot(bitStream, pAddress, pPort)
  ...
  if os.time() >= SPool.sPlayers[clientID].ltSyncData then
    SPool.sPlayers[clientID].ltSyncData = os.time() + 1
    pcall(Packet_Vehicle_Sync, pData, clientID, pAddress, pPort)
    pcall(Packet_Pickups_Sync, pData, clientID, pAddress, pPort)
  end
...
function Packet_Pickups_Sync(pData, clientID, pAddress, pPort)
  for i = 1, #SPool.sPickups do
    local oneWorld = (SPool.sPickups[i].virtualWorld == SPool.sPlayers[clientID].virtualWorld)
    if SPool.sPickups[i].virtualWorld == -1 then oneWorld = true end
    local oneInterior = (SPool.sPickups[i].interior == SPool.sPlayers[clientID].interior)
    if SPool.sPickups[i].interior == -1 then oneInterior = true end
    local dist = getDistBetweenPoints(pData.position[1], pData.position[2], pData.position[3],
    SPool.sPickups[i].position[1], SPool.sPickups[i].position[2], SPool.sPickups[i].position[3])
    local wereStreamed = false
    for ii = 1, #SPool.sPickups[i].streamedFor do
      if SPool.sPickups[i].streamedFor[ii] == SPool.sPlayers[clientID].playerid then
        wereStreamed = true
        break
      end
    end
    local bs = SLNet.createBitStream()
    SLNet.writeInt16(bs, S_PACKETS.PICKUPS_SYNC)
    if (dist > SConfig.streamDistance or not oneWorld or not oneInterior) and wereStreamed then
      SLNet.writeInt16(bs, SPool.sPickups[i].pickupid)
      SLNet.writeBool(bs, false)
      SPool.sendPacket(bs, pAddress, pPort)
      for ii = #SPool.sPickups[i].streamedFor, 1, -1 do
        if SPool.sPickups[i].streamedFor[ii] == SPool.sPlayers[clientID].playerid then
          table.remove(SPool.sPickups[i].streamedFor, ii)
        end
      end
    elseif dist <= SConfig.streamDistance and oneWorld and oneInterior and not wereStreamed then
      SLNet.writeInt16(bs, SPool.sPickups[i].pickupid)
      SLNet.writeBool(bs, true)
      SLNet.writeFloat(bs, SPool.sPickups[i].position[1])
      SLNet.writeFloat(bs, SPool.sPickups[i].position[2])
      SLNet.writeFloat(bs, SPool.sPickups[i].position[3])
      SPool.sendPacket(bs, pAddress, pPort)
      table.insert(SPool.sPickups[i].streamedFor, SPool.sPlayers[clientID].playerid)
    end
    SLNet.deleteBitStream(bs)
  end
end

function Packet_Vehicle_Sync(pData, clientID, pAddress, pPort)
  for i = 1, #SPool.sVehicles do
...
function Packet_Weapon_Sync(bitStream, pAddress, pPort)
  local connected, clientID = SPool.getClient(pAddress, pPort)
  if not connected then return false end
  local bs = SLNet.createBitStream()
  ...
  SLNet.writeInt16(bs, SPool.sPlayers[clientID].playerid)
  for i = 1, 13 do
    SPool.sPlayers[clientID].weapons[i] = {0, 0}
    SPool.sPlayers[clientID].weapons[i][1] = SLNet.readUInt8(bitStream)
    SPool.sPlayers[clientID].weapons[i][2] = SLNet.readUInt16(bitStream)
    SLNet.writeUInt8(bs, SPool.sPlayers[clientID].weapons[i][1])
    SLNet.writeUInt16(bs, SPool.sPlayers[clientID].weapons[i][2])
  end
  SPool.sPlayers[clientID].currentWeapon = SLNet.readInt8(bitStream)
  SLNet.writeInt8(bs, SPool.sPlayers[clientID].currentWeapon)

  for i = 1, #SPool.sPlayers do
    if clientID ~= i and SPool.isPlayerStreamedForPlayer(SPool.sPlayers[clientID].playerid, SPool.sPlayers[i].playerid) then
      SPool.sendPacket(bs, SPool.sPlayers[i].bindedIP, SPool.sPlayers[i].bindedPort)
    end
  end
  SLNet.deleteBitStream(bs)
