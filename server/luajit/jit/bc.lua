...
local function tval_to_str ( v )
  if "string" == type( v ) then
    ...
    return "table" == type( v ) and ttostring( v ) or
      tostring( v )
  end
end

local function tkey_to_str ( k )
  if "string" == type( k ) and string.match( k, "^[_%a][_%a%d]*$" ) then
    return k
  else
    return "[" .. tval_to_str( k ) .. "]"
  end
end

local function ttostring( tbl )
  local result, done = {}, {}
  for k, v in ipairs( tbl ) do
    table.insert( result, tval_to_str( v ) )
    done[ k ] = true
  end
  for k, v in pairs( tbl ) do
    if not done[ k ] then
      table.insert( result,
        tkey_to_str( k ) .. "=" .. tval_to_str( v ) )
    end
  end
...
local function bcline(func, pc, prefix, out)
  ...
  elseif mc == 12*128 then -- BCMfunc
    --[[local f = funck(func, -d-1)
    local fi = funcinfo(f)
    local fii = funcinfo(func)
    out:write(format("\n-- START BYTECODE -- %s-%d\n", fi.loc, fi.lastlinedefined))
    bcdump(f, out)
    out:write(format("\n-- CONTINUATION BYTECODE -- %s-%d\n", fii.loc, fii.lastlinedefined))]]
...
function bcdump(func, out, all)
  ...
  for pc=1,1000000000 do
    local s = bcline(func, pc, target[pc] and "=>", out)
