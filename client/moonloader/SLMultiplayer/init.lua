...
function checkPlayerState()
  if isCharInAnyCar(PLAYER_PED) then
    ...
    for i = 1, #GPool.GVehicles do
      if GPool.GVehicles[i].handle and GPool.GVehicles[i].handle == car then
        ...
        if LPlayer.lpPlayerState == S_PLAYERSTATE.PS_ONFOOT then
          ...
          SLNet.writeInt16(bs, S_RPC.ENTER_VEHICLE)
          SLNet.writeInt16(bs, tonumber(GPool.GVehicles[i].vehicleid))
          SLNet.writeInt8(bs, tonumber(carSeat))
          SPool.sendRPC(bs)
    ...
    or LPlayer.lpPlayerState == S_PLAYERSTATE.PS_PASSANGER then
      ...
      SLNet.writeInt16(bs, tonumber(LPlayer.lpVehicleID))
      SLNet.writeInt8(bs, tonumber(LPlayer.lpVehicleSeat))
      SPool.sendRPC(bs)
      SLNet.deleteBitStream(bs)
    end
